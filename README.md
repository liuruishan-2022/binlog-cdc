...
1. 目前的压测性能,发现通过channel的模式来处理,看到Kafka消息的投递能够从2w/s-->2.9w/s到3w/s左右
2. 目前线下压测,发现CPU能够使用到:1.8c-2c的样子.不过还是需要查找下如何提高吞吐量
3. binlog的解析也能够从38s降低到29s/30s的样子,这样子看提升了:22%的样子

## 1.1.3 版本 (2025-03-09)

**修复:**

1. 修复了之前的重连的bug,现在的做法是直接退出，以pod的重启来做
2. 然后修复了之前的expect("")的写法，会造成panic,现在改称Result<>的处理方式
3. 修改mysql binlog的读取的超时时间设置为可配置，之前10s容易出现问题
...
1. 支撑基本的binlog cdc同步到Kafka指定的Topic
2. 支持自定义函数: LOCALTIMESTAMP,以及 "" as 或者是:-1 as 这种解析
3. 支持最新采集的binlgo文件名字写入到savepoint文件夹中
